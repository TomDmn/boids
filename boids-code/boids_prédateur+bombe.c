...
#define NB_BOMB 5
#define W_BOMB 1.0            // coefficient du poids de la bombe
...
#define BOMB_EXPLOSION_TICKS 15
#define BOMB_TICK_MIN 700
#define BOMB_TICK_RANGE 800
...
static void draw_filled_circle(SDL_Renderer* r, int cx, int cy, int radius){ // helper pour dessiner une bombe ronde
    for (int dy = -radius; dy <= radius; dy++){
        int w = (int)sqrt((double)(radius * radius - dy * dy));
...
static void draw_filled_triangle(SDL_Renderer* r, double x1, double y1, double x2, double y2, double x3, double y3){
    double miny = fmin(y1, fmin(y2, y3));
    double maxy = fmax(y1, fmax(y2, y3));
    int start_y = (int)floor(miny);
    int end_y = (int)ceil(maxy);
    for (int y = start_y; y <= end_y; y++){
        double scan_y = (double)y + 0.5; // centre de scanline
        double xs[3];
        int count = 0;
        if ((scan_y >= fmin(y1, y2)) && (scan_y <= fmax(y1, y2)) && y1 != y2){
            xs[count++] = x1 + (scan_y - y1) * (x2 - x1) / (y2 - y1);
        }
        ...
        if (count < 2) continue;
        double xa = xs[0], xb = xs[1];
        if (xa > xb){
            double t = xa; xa = xb; xb = t;
        }
        SDL_RenderDrawLine(r, (int)floor(xa), y, (int)ceil(xb), y);
    }
}
...
fx FX_FLEE_DIST2, FX_KILL_DIST2;
fx FX_W, FX_H, FX_HALF_W, FX_HALF_H;
fx FX_W_BOMB;
...
static void Reynold_Pred(Boid *b, predator* pred, fx* axs, fx* ays){  // calcule les accélérations selon les règles de Reynolds et l'évitement du prédateur
    ...
    for (int dx = -1; dx <= 1; dx++){             // pour chaque case voisine dans la grille de hachage
        for (int dy = -1; dy <= 1; dy++){
            ...
            for(hashed* h = grille[case_x][case_y]; h; h = h->next){ // pour chaque boid dans la case
                ...
                fx rx = b1->x - b->x;                    
                if (rx > FX_HALF_W) rx -= FX_W;       //Prise en compte de l'effet de tore, si la distance est plus grande que la moitié de la largeur, on soustrait la largeur
                if (rx < -FX_HALF_W) rx += FX_W;      //pour calculer la distance entre deux points sur un tore

                fx ry = b1->y - b->y;
                if (ry > FX_HALF_H) ry -= FX_H;
                if (ry < -FX_HALF_H) ry += FX_H;
    ...
    *ays = ay_ali + ay_coh + ay_sep;
    ...
    fx px = pred->x - b->x;
    fx py = pred->y - b->y;
    if (px > FX_HALF_W) px -= FX_W;  // prise en compte de l'effet de tore
    if (px < -FX_HALF_W) px += FX_W;
    if (py > FX_HALF_H) py -= FX_H;
    if (py < -FX_HALF_H) py += FX_H;

    int64_t dist_pred2 = fx_norm2_64(px, py); 

    if (dist_pred2 < FX_KILL_DIST2){  // si le boid est trop proche du prédateur, il meurt
        b->vie = 0;
    ...
    if (dist_pred2 < FX_FLEE_DIST2){  // si le boid est dans la zone de fuite du prédateur
...
static void update_predator(predator* pred){  // met à jour la position et la vitesse du prédateur
    ...
    /* gestion des bords (effet de tore)*/
    if (pred->x < 0) pred->x += FX_W;
    if (pred->x >= FX_W) pred->x -= FX_W;
    if (pred->y < 0) pred->y += FX_H;
    if (pred->y >= FX_H) pred->y -= FX_H;
}
...
static void init_constants(){             //calcul des constantes en point fixe, on les calcule une seule fois au début
    ...
    FX_KILL_DIST2 = fx_mul(kill_r, kill_r);
    FX_FLEE_DIST2 = fx_mul(flee_r, flee_r);
...
static void draw_predator(SDL_Renderer* r, fx x, fx y, fx vx, fx vy){
    ...
    if (vx == 0 && vy == 0) heading = -M_PI_2; // orientation par défaut vers le haut
    double size = 18.0;
...
int main() {
    ...
    TTF_Init(); // initialisation de SDL_ttf pour le rendu de texte
    ...
    while (running) {
        ...
        for (int i = 0; i < NB_BOMB; i++){
            int bx = (int)((bombs[i].explosion_timer > 0 ? bombs[i].blast_x : bombs[i].x) >> FX_SHIFT); // position de l'explosion si en train d'exploser, sinon position de la bombe
            ...
            if (bombs[i].explosion_timer > 0){   // si la bombe est en train d'exploser, on dessine le blast
                double t = 1.0 - (double)bombs[i].explosion_timer / (double)BOMB_EXPLOSION_TICKS; // progression de l'anim
                int base_rad = (int)sqrt((double)bombs[i].radius2 / (double)FX_ONE);
                int rad = base_rad + (int)(t * base_rad * 0.35);  // expansion légère
                Uint8 alpha = (Uint8)(200 * (1.0 - t));
                // halo extérieur
                SDL_SetRenderDrawColor(r,255,140,0,alpha);
                draw_filled_circle(r, bx, by, rad);
                // cœur lumineux
                SDL_SetRenderDrawColor(r,255,220,120,alpha);
                draw_filled_circle(r, bx, by, rad/2);
            }
        ...
        for (int i = 0; i < N_BOIDS; i++) {
            if (!boids[i].vie) continue;
            Reynold_Pred(&boids[i], &pred,  &boids[i].ax, &boids[i].ay);
            update(&boids[i]);
            SDL_SetRenderDrawColor(r,255,255,255,255);
            draw_arrow(r, boids[i].x, boids[i].y, boids[i].vx, boids[i].vy, 8.0);
        }
        
        update_predator(&pred);
        draw_predator(r, pred.x, pred.y, pred.vx, pred.vy);

        // keyboard control
        const Uint8* state = SDL_GetKeyboardState(NULL);

        fx react = fx_from_double(PRED_REACT);
        pred.ax = 0;
        pred.ay = 0;

        if (state[SDL_SCANCODE_UP])    pred.ay -= react;
        if (state[SDL_SCANCODE_DOWN])  pred.ay += react;
        if (state[SDL_SCANCODE_LEFT])  pred.ax -= react;
        if (state[SDL_SCANCODE_RIGHT]) pred.ax += react;
        ...
        if (elapsed_ticks < target_ticks){
            ...
            // boucle d'attente fine pour combler le delta restant
